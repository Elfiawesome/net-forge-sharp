...
public class PacketStream
{
	...
	// Size of the field that stores the total length of (Id + Payload). [2]
	const int PACKET_LENGTH_FIELD_SIZE = sizeof(int);
	// Size of the field that stores the packet Id [3]
	const int PACKET_ID_FIELD_SIZE = sizeof(ushort);
	// Max size for (Id + Payload) [2MB]
		...
		try
		{
			...
			using (var ms = new MemoryStream(typeAndPayloadBuffer))
			{
				using (var br = new BinaryReader(ms, _stringEncoding))
				{
					...
					BasePacket? packet = BasePacket.REGISTRY.Get(packetId)?.Invoke();
					if (packet == null)
					{
